...
import { ArrowLeft, Mail, Trash2, Loader2, ExternalLink } from "lucide-react";
...
function getField(data: Record<string, unknown>, ...keys: string[]): string {
  for (const k of keys) {
    const v = data[k] || data[k.toLowerCase()] || data[k.toUpperCase()];
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            {submissions.map((sub) => {
              const name  = getField(sub.data, "name", "fullName", "full_name") || "Anonymous";
              const email = getField(sub.data, "email");
              const msg   = getField(sub.data, "message", "msg", "body");
              return (
                <div key={sub.id} onClick={() => handleOpen(sub)}
                  ...
                  {msg && <p className="text-xs text-slate-500 truncate mt-1 ml-11">{msg}</p>}
                  <p className="text-xs text-slate-400 mt-1 ml-11">
                    {sub.formName && <span className="mr-2 text-josett-400">{sub.formName}</span>}
          ...
          {selected ? (
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-6 h-fit sticky top-6">
              {(() => {
                const name  = getField(selected.data, "name", "fullName") || "Anonymous";
                const email = getField(selected.data, "email");
                const phone = getField(selected.data, "phone");
                const msg   = getField(selected.data, "message", "msg", "body");
                const extras = Object.entries(selected.data).filter(
                  ([k]) => !["name","fullName","full_name","email","phone","message","msg","body"].includes(k)
                );
                return (
                  <>
                    ...
                    <div className="space-y-3">
                      {email && (
                        <div>
                          ...
                          <a href={`mailto:${email}`} className="text-sm text-josett-600 dark:text-josett-400 flex items-center gap-1 hover:underline">
                      ...
                      {phone && (
                        <div>
                          ...
                          <a href={`tel:${phone}`} className="text-sm text-josett-600 dark:text-josett-400 hover:underline">{phone}</a>
                        </div>
                      )}
                      {msg && (
                        <div>
                          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1">Message</p>
                          <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed bg-slate-50 dark:bg-slate-800 rounded-xl p-3">{msg}</p>
                        </div>
                      )}
                      {extras.map(([k, v]) => (
                        <div key={k}>
