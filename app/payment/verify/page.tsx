    ...
    fetch("/api/payments/verify", {
      ...
      .then((data) => {
        if (data.success) {
          ...
          // Redirect to site dashboard after 3 seconds
          setTimeout(() => {
            router.push(data.siteId ? `/sites/${data.siteId}` : "/sites");
          }, 3000);
        } else {
          setStatus("failed");
        ...
        {status === "success" && (
          <>
            ...
            {siteId && (
              <Link
                href={`/sites/${siteId}`}
                className="inline-flex items-center gap-2 bg-josett-600 text-white font-bold px-6 py-3 rounded-xl hover:bg-josett-700 transition-all"
              >
                Go to My Site
              </Link>
            )}
        ...
        {status === "failed" && (
          <>
            ...
            <div className="flex gap-3 justify-center">
              <Link href="/sites/new" className="bg-josett-600 text-white font-bold px-5 py-2.5 rounded-xl hover:bg-josett-700 transition-all text-sm">
                Try Again
              </Link>
              <Link href="/dashboard" className="border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 font-semibold px-5 py-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-all text-sm">
                Dashboard
              </Link>
            </div>
          </>
